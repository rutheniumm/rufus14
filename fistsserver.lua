...
local tool = owner.Backpack:FindFirstChild("Fists")
local owner
local charhum
local character
local charroot
...
local animrawdata = { --had to :inverse() rootpart on every keyframe because i put torso as part0 for no literal reason
	...
	["idle"] = {
		[1] = {
			["rarmweld"] = {CFrame.new(1.14219666, -0.656560421, -0.909226418, 0.941444933, 0.325054795, -0.089532733, 0.258639753, -0.866617441, -0.426701248, -0.216293022, 0.378559977, -0.899948895), easingstyles.Sine, easingdirs.Out, 0.3},
			["larmweld"] = {CFrame.new(-1.13857365, -0.698836803, -0.871101141, 0.9392398, -0.32580018, 0.108070657, -0.246695966, -0.859613478, -0.447438568, 0.238674939, 0.393592596, -0.887760043), easingstyles.Sine, easingdirs.Out, 0.3},
...
local notable_nerve_response = { --the code where it makes you DIE
	["Head"] = function(part, thehum)
		...
		if not thehum.PlatformStand and thehum:GetState() ~= Enum.HumanoidStateType.FallingDown then
			...
			part.AssemblyAngularVelocity = (charroot.CFrame.LookVector * -hitvector[swingdatas[swinganimation][1]]*5) + (charroot.CFrame.UpVector * -hitvector[swingdatas[swinganimation][1]]*15)
...
local remotebehavior = {
	["lmb"] = function(pos)
		if equipped() and toolstate == "idle" then
			...
			task.wait(0.05)
			--the end
			if currentanimtick == btick or currentequiptick ~= beqtick then
				toolstate = "idle"
				remote:FireClient(owner, "isholding")
			end
		end
	...
	["part"] = function(thepart, thepos, thenormal)
		--print("hit attempt")
		if equipped() and toolstate ~= "idle" and toolstate ~= "block" and thepart then
...
tool.Equipped:Connect(function()
	...
	runfunc = runservice.Stepped:Connect(function(_, delta)
		...
		currentwelds["lookrootweld"].C0 = currentwelds["lookrootweld"].C0:lerp(CFrame.new(0,1+lookpartheight,0) * CFrame.Angles(-unitrel.y/1.5,theXtilt/1.5,0),delta*10)
	end)
	task.spawn(function()
